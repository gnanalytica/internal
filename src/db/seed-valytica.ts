...
/**
 * Valytica's FIXED monthly operating costs as expenses on the Valytica project
 * (its Finance department). Every line traces to a real service wired into the
 * valytica repo (package.json deps + .env keys) — nothing invented:
 *
...
async function seedCosts(projectId: string, workspaceId: string): Promise<void> {
  // Idempotent: clear every vendor this script manages (incl. dropped ones) for
  // Valytica, then re-insert the current set.
  await db
    .delete(schema.expenses)
    .where(and(eq(schema.expenses.projectId, projectId), inArray(schema.expenses.vendor, MANAGED_VENDORS)));
  ...
  console.log(`Costs:      ${COSTS.length} monthly cost lines seeded.`);
...
async function main() {
  ...
  // 8. Seed fixed monthly operating costs
  await seedCosts(project.id, ws.id);

  console.log(`\nDone. Valytica canonical seed complete: ${MILESTONES.length} milestones, ${FEATURES.length} features, ${pgTotal} pages, ${COSTS.length} expenses.`);
}
