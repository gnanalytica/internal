...
/** Gate verdict on a milestone — the weekly green/amber/red call. */
export function MilestoneStatusChip({ status }: { status: string }) {
  const s = MILESTONE_STATUS_MAP[status as MilestoneStatusId];
  if (!s) return null;
  return (
    <span
      className="inline-flex items-center gap-1 rounded-full border px-1.5 py-0.5 text-[10px] font-medium"
...
export function MilestoneStatusPicker({
  ...
  value: string;
  onChange: (v: MilestoneStatusId) => void;
}) {
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        className="rounded-md focus:outline-none focus-visible:ring-2 focus-visible:ring-ring/50"
        aria-label="Set milestone status"
      >
        <MilestoneStatusChip status={value} />
      </DropdownMenuTrigger>
      <DropdownMenuContent align="start" className="w-40">
        {MILESTONE_STATUSES.map((s) => (
          <DropdownMenuItem key={s.id} onClick={() => onChange(s.id)} className="gap-2 text-xs">
            <Dot color={s.color} />
            <span className="flex-1">{s.label}</span>
            {value === s.id && <Check className="size-3.5 opacity-70" />}
          </DropdownMenuItem>
        ))}
      </DropdownMenuContent>
    </DropdownMenu>
