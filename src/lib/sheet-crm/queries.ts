...
/**
 * The schema arrives by `pnpm db:push`, the code arrives by a Vercel deploy,
 * and the two are not one atomic step. Between them these tables do not
 * exist, and an unguarded read turns the whole Prospects surface into a 500 —
 * which reads to whoever opens it as data loss rather than as a setup step
 * that has not been run.
 *
 * So every read here degrades to "nothing yet" on Postgres 42P01
 * (undefined_table) / 42703 (undefined_column), and the surface says it is
 * not set up. Any other error still throws: a real outage must not be
 * disguised as an empty workspace.
 */
...
export const SHEET_SCHEMA_MISSING = Symbol("sheet schema missing");
...
async function orEmpty<T>(run: () => Promise<T>, fallback: T): Promise<T> {
  try {
    ...
    if (isMissingSchemaError(err)) {
      console.warn("[sheet-crm] schema not pushed yet; run `pnpm db:push`");
      return fallback;
    }
    throw err;
  }
...
/** Whether the sync tables exist. Drives the "not set up" state. */
export async function sheetSchemaReady(): Promise<boolean> {
  try {
    await db.select({ n: sql<number>`1` }).from(sheetRows).limit(1);
    return true;
  } catch (err) {
