...
import { callClaudeJson, isAiConfigured } from "@/lib/ai";
...
export async function proposeIssuesFromPage(
  ...
  // The shape is the schema's job now, so the prompt only has to say what makes
  // a good issue. It used to carry "Return ONLY a JSON array… No prose outside
  // the JSON", which the model could disregard without anything noticing.
  const out = await callClaudeJson<{ issues: { title: string; description: string }[] }>({
    schema: {
      type: "object",
      properties: {
        issues: {
          ...
          items: {
            ...
            properties: {
              ...
              description: { type: "string", description: "1-3 sentences of context." },
            },
...
/**
 * Retrieve the docs and issues a question should be answered from (keyword RAG).
 *
 * Retrieval only — the answer is generated by `/api/ask`, which streams it. The
 * two were one function returning `{answer, sources}`, which meant the reader
 * watched "Thinking…" for the whole generation and then got everything at once.
 */
export async function askContext(
  question: string,
): Promise<{ sources: import("@/lib/types").AskSource[]; blocks: string[]; note?: string }> {
  if (!isAiConfigured()) {
    throw new Error("AI isn't configured. Add an ANTHROPIC_API_KEY to enable this.");
  }
  const q = question.trim();
  if (!q) return { sources: [], blocks: [], note: "" };
  const ws = await getWorkspace();
  ...
  if (blocks.length === 0) {
    return { sources: [], blocks: [], note: "I couldn't find anything relevant in this workspace." };
  }
