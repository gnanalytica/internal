...
1. **Column-shift corruption** (values displaced one column): Companies C0014, C0039, C0041, C0042, C0045 (`linkedin` holds headcount text, `num_valuers` holds addresses, `research_confidence` holds URLs, `sales_angle` holds `High`/`Medium`); Prospect Intelligence row for Chinta Venkata Raghupati (`Last Researched` holds prose, `Source URLs` holds a date) and rows 18–22 are orphan fragments of the Badam record; Deep Dive row for Badam shifted from `Likely Trigger` onward; GTM Personas rows 2–16 shifted left from `What We Want` (`Research Criteria` empty, criteria text sits in `What To Avoid`).
...
3. **`duplicate_match_ids` misuse**: holds the sentence `MATCH ON IBBI / EMAIL / PHONE / COMPANY — MERGE REQUIRED` instead of an id (P00038, P00050); the README says it holds the first canonical id.
...
6. **Prospects without a `person_id`**: 10 of 21 in Prospect Intelligence (Sudheer Chekka, Raghupati, Saripalli Srinivasa Raju, H V Anjaneya Prasad, Badam, Kotagiri Chiranjeevi, Kurumilla Indrasena Chary, Talluri Venkata Ramana, Gouru Neelakanta Reddy, Addagiri Srihari Babu) plus the bank-side / institutional rows (Rohit Kumar, Mandari Yadaiah, Dhurjati Patri, Gopi Krishna B, the two SBI / LIC HFL "decision path" rows) which have no home in People at all. The README's own rule says new discoveries "must be added to People/Companies with a permanent ID once verified".
...
b. **Assign `person_id`s to the 10 prospects without one** (§1.5 item 6) by appending them to People per the README rule, and decide where the bank-side people (Rohit Kumar etc.) live — People with a role marker, or a new `Institution Contacts` tab. Fallback: name-matched links flagged "weak".
